...
01) create a main function.
02) in the main function, create a variable called "novel_url_list", that contains a list of website urls from project
    gutenberg, for at least three novels of your choice. In addition, in the main function create the following empty
    lists: 01) novel_tokens_list, 02) novel_fdist_list.
03) create a function called "get_novel()" that takes a single novel URL as an input, and uses the code from
    try_nltk.py to download and import the text from that novel. This function should return two variables: 01) the
    novel's text as a lower-cased NLTK token_list, 02) an NLTK fdist object for the novel
04) in the main function, create a for loop that iterates through every URL in the novel_url_list. In the for loop,
...
06) create a function called "plot_most_freq_word()", that takes the novel_fdist_list as an input. this function should
    create a maplotlib multiple plot figure that has 01 column and n rows, where n is the number of novels in fdist_list.
