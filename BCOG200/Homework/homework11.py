...
2) in the main function, create a variable called "novel_url_list", that contains a list of website urls from project
...
3) create a function called "get_novel()" that takes a single novel URL as an input, and uses the code from
    try_nltk.py to download and import the text from that novel. This function should return two variables: 1) the
    novel's text as a lower-cased NLTK token_list, 2) an NLTK fdist object for the novel
4) in the main function, create a for loop that iterates through every URL in the novel_url_list. In the for loop,
    call the "get_novel()" function, and add the resulting token lists and fdists to novel_tokens_list and
    novel_fdist_list.
5) create a function called "plot_freq_dists()" that takes novel_fdist_list as an input. this function should create
    ...
    function.
6) create a function called "plot_most_freq_word()", that takes the novel_fdist_list as an input. this function should
    create a maplotlib multiple plot figure that has 1 column and n rows, where n is the number of novels in fdist_list.
...
7) add a second argument to the plot_most_freq_word() function specifying whether stop words should be removed, set to
...
8) add a third argument to the plot_most_freq_word() function, that allows you to specify a part of speech, If this
...
7) make sure that plt.show() is not called until the very end, so that all plots appear at the same time.
"""