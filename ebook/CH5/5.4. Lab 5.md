...
def q8_9_10():
    ...
    - Q9: loop through "animal_sizes" dictionary below
        - check to see if the animal is in the animal_data_dict
        - if it is, create a new key in that animal's dictionary called "size", which points to the corresponding value
    - Q10: loop through the animal_data_dict, and print out each animal's formal name, common name, and size on each 
        line
