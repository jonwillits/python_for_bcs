...
"""
We start by importing our modules at the top of the file. Each import line makes the code from that module available in 
your program, as if you had copied and pasted all the code from that file (like sys.py) here in your script.
"""
import sys
import os
import math
...
"""
Next, I have created a function for each module, which demonstrates some of the useful things that module can be used to
do.
"""
...
def demonstrate_os_module():
    ...
    path_to_a_directory = '../03-01/lyrics/'
    try:
    ...
    # comment and run these lines one at a time so you can see them in action. Have your Assignments/05/ folder open
    # so you can see the new folders being created and renamed and removed.
...
def demonstrate_math_module():
    ...
    of decimal places, and sometimes we don't care about the difference between 0.00000000002 and 0.00000000001. So 
    python (and most programming languages) take shortcuts that can result in imprecision and rounding errors.  This is 
    something to keep in mind if the 10th place after the decimal really matters in your situation.
    """
    print("")
...
def demonstrate_time_module():
    """
    The time module gives us access to date and time information.
    ...
    elapsed since some arbitrary time. On Mac/Linux, it is the number of seconds since January 1, 1970, 00:00:00 (UTC). 
    Why would you care? Well, it is useful if you want to keep track of how long something took. We often call this 
    "profiling" our code, when we are concerned about performance"""
    ...
    print("Adding {} numbers took {:.6f} seconds".format(iterations, elapsed_time))
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    print("Multiplying {} numbers took {:.6f} seconds".format(iterations, elapsed_time))
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    print("Appending {} items to a list took {:.6f} seconds".format(iterations, elapsed_time))
    ...
    end_time = time.time()
    elapsed_time = end_time - start_time
    print("A loop that did nothing {} times took {:.6f} seconds".format(iterations, elapsed_time))
    print("")
...
def demonstrate_random_module():
    ...
    """
    The code above randomly sampled "with replacement", in other words, every time we picked a random item, it was
    reinserted in the list before the next random element was taken. If we wanted to choose five random items "without
    replacement", how would we do that?
    
    CODE REQUIRED
    """

    """
    Continue commenting these lines.
    COMMENT REQUIRED
    """
    number_list.sort()
